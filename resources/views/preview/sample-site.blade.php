@php
    ...
    $reviewCount = $lead->review_count ?: '120+';
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@500;700;900&family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&family=Playfair+Display:ital,wght@0,500;0,700;1,400&family=Space+Mono:wght@400;700&display=swap" rel="stylesheet">
    <script>
        tailwind.config = {
            theme: {
                extend: {
...
<body class="{{ $fontFamily }} antialiased text-slate-800 bg-white selection:bg-slate-900 selection:text-white">
    ...
    <!-- Sticky Floating "Claim This Website" Header Ribbon (Independent of dynamic styling) -->
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 flex flex-col sm:flex-row items-center justify-between gap-2.5 text-xs">
            <div class="flex items-center gap-2 text-center sm:text-left">
                ...
                <span class="text-slate-300">
                    Designed exclusively for <strong class="text-white">{{ $bizName }}</strong>
                </span>
            </div>
    ...
    <!-- Main Navigation Bar -->
    <header class="bg-white/95 backdrop-blur-md border-b border-slate-100 transition-all">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-20">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @else
                        <div class="w-10 h-10 rounded-xl {{ $primaryColor }} flex items-center justify-center text-white font-extrabold text-lg shadow-md">
                            {{ strtoupper(substr($bizName, 0, 1)) }}
                        </div>
                    @endif
                    <div>
                        <a href="#home" class="text-xl font-bold tracking-tight text-slate-900 hover:opacity-90 transition-opacity">
                            {{ $bizName }}
                        </a>
                ...
                <nav class="hidden md:flex items-center gap-8 text-sm font-semibold text-slate-600">
                    ...
                    <a href="#about" class="hover:text-slate-900 transition-colors">About Us</a>
                ...
                <div class="flex items-center gap-3">
                    @if($lead->phone)
                        <a href="tel:{{ $lead->phone }}" class="hidden sm:inline-flex items-center gap-2 text-sm font-semibold text-slate-700 hover:text-slate-900 px-3 py-2">
                            <svg class="w-4 h-4 {{ $textColor }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                            {{ $lead->phone }}
                        </a>
                    @endif
                    <a href="#contact" class="inline-flex items-center justify-center px-5 py-2.5 rounded-xl text-sm font-semibold text-white {{ $primaryColor }} hover:opacity-95 shadow-md transition-all transform hover:-translate-y-0.5">
                        {{ $primaryCta }}
                    </a>
                </div>
            </div>
    ...
    <main id="home">
        <!-- Dynamic Hero Section (Conditional Rendering based on $layout) -->

        {{-- 1. LAYOUT: split-with-form (e.g. Garages, Contractors, Service Providers) --}}
                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                        <div class="lg:col-span-7 text-center lg:text-left">
                            ...
                            <div class="mt-8 pt-8 border-t border-slate-200 flex flex-wrap items-center justify-center lg:justify-start gap-5 text-xs sm:text-sm font-semibold text-slate-700">
                        ...
                        <!-- Right Column: Fake Lead Capture Form -->
                        <div class="lg:col-span-5">
                            <div class="bg-white rounded-3xl p-6 sm:p-8 shadow-xl border border-slate-200/80 relative">
                                ...
                                <form onsubmit="event.preventDefault(); alert('Demo inquiry received for {{ $bizName }}!');" class="space-y-4">
                                    <div>
                                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Full Name</label>
                                        <input type="text" placeholder="John Smith" required class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-slate-900">
                                    </div>
                                    <div>
        ...
        {{-- 2. LAYOUT: gallery-grid (e.g. Real Estate, Restaurants, Venues, Salons) --}}
            ...
            <section class="relative pt-12 pb-20 md:pt-16 md:pb-24 bg-white border-b border-slate-100">
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        @foreach($nicheFeatures as $idx => $feat)
                            <div class="group relative rounded-2xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 bg-slate-900 text-white min-h-[220px] flex flex-col justify-end p-6">
                    ...
                    <div class="mt-10 flex flex-wrap items-center justify-center gap-8 text-sm font-semibold text-slate-700">
                        @foreach($trustIndicators as $trust)
                            <div class="flex items-center gap-2">
        ...
        {{-- 3. LAYOUT: centered-bold (e.g. Lawyers, Tech, Agencies, Consultants) --}}
        @else
            <section class="relative pt-16 pb-24 md:pt-24 md:pb-28 text-center bg-gradient-to-b from-slate-50 via-white to-white border-b border-slate-100">
                <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-3xl mx-auto pt-10 border-t border-slate-200">
                        @foreach($trustIndicators as $trust)
                            <div class="p-4 rounded-xl bg-white border border-slate-100 shadow-sm text-center">
                                <div class="text-emerald-600 font-extrabold mb-1">✓ Verified</div>
                                <div class="text-xs sm:text-sm font-semibold text-slate-700">{{ $trust }}</div>
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        <!-- Dynamic Niche Features & Services Grid -->
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center max-w-3xl mx-auto mb-16">
                    ...
                    <p class="text-slate-600 text-base sm:text-lg">
                        Engineered specifically for clients of {{ $bizName }} in {{ $city }}.
                    </p>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    @forelse($nicheFeatures as $index => $feature)
                        <div class="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-200 flex flex-col justify-between">
                            <div>
                                <div class="w-12 h-12 rounded-xl bg-slate-50 {{ $textColor }} flex items-center justify-center mb-6 border border-slate-100">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                    <div class="lg:col-span-6">
                        <div class="p-8 sm:p-12 rounded-3xl bg-slate-950 text-white shadow-2xl relative overflow-hidden">
                            <div class="relative z-10">
                                <span class="inline-block px-3 py-1 rounded-full bg-white/15 text-xs font-bold uppercase tracking-wider mb-6">
                                    Proven Reputation
                                </span>
                                <h3 class="text-2xl sm:text-3xl font-extrabold mb-4 leading-snug">
                                    Delivering Quality {{ $category }} Across {{ $city }}
                                </h3>
                                ...
                                <div class="grid grid-cols-2 gap-4 pt-6 border-t border-slate-800">
                                    <div>
                                        ...
                                        <div class="text-xs text-slate-400 mt-1">Google Score</div>
                                    </div>
                                    <div>
                    ...
                    <div class="lg:col-span-6">
                        <span class="text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full bg-slate-100 {{ $textColor }}">
                            Why Work With Us
                        </span>
                        ...
                        <div class="space-y-4">
                            @foreach($trustIndicators as $trustPoint)
                                <div class="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
                                    <div class="w-6 h-6 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center flex-shrink-0 text-xs font-bold mt-0.5">
                                        ✓
                                    </div>
                                    <div>
                                        <div class="text-sm font-bold text-slate-900">{{ $trustPoint }}</div>
                                        <div class="text-xs text-slate-500 mt-0.5">Dedicated standard practiced across all engagements with {{ $bizName }}.</div>
                                    </div>
                                </div>
                            @endforeach
                        </div>
        ...
        <section id="contact" class="py-20 bg-slate-50 border-t border-slate-200">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="max-w-3xl mx-auto text-center mb-16">
                    <span class="text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full bg-white border border-slate-200 {{ $textColor }}">
                        Connect
                    </span>
                    <h2 class="text-3xl sm:text-4xl font-extrabold text-slate-900 mt-4 mb-4">
                        Contact {{ $bizName }}
                    ...
                    <p class="text-slate-600 text-base sm:text-lg">
                        Ready to book or have questions? Get in touch with our team today.
                    </p>
                </div>

                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    <div class="lg:col-span-5 bg-white rounded-2xl p-8 border border-slate-200 shadow-sm space-y-6">
                        <h3 class="text-xl font-bold text-slate-900">Direct Contact</h3>

                        @if($lead->phone)
                            <div class="flex items-start gap-4">
                                <div class="w-10 h-10 rounded-xl bg-slate-50 {{ $textColor }} flex items-center justify-center flex-shrink-0 border border-slate-100">
                        ...
                        @if($primaryEmail)
                            <div class="flex items-start gap-4">
                                <div class="w-10 h-10 rounded-xl bg-slate-50 text-emerald-600 flex items-center justify-center flex-shrink-0 border border-slate-100">
                        ...
                        @if($lead->address)
                            <div class="flex items-start gap-4">
                                <div class="w-10 h-10 rounded-xl bg-slate-50 text-rose-600 flex items-center justify-center flex-shrink-0 border border-slate-100">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                </div>
                                <div>
                    ...
                    <div class="lg:col-span-7 bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
                        ...
                        <form onsubmit="event.preventDefault(); alert('Inquiry sent!');" class="space-y-4">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Your Name *</label>
                                    <input type="text" required class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Contact Details *</label>
                                    <input type="text" required placeholder="Email or Phone" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900">
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Message *</label>
                                <textarea rows="4" required placeholder="How can we assist you?" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900"></textarea>
                            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row items-center justify-between gap-6">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-1">Professional {{ $category }} in {{ $city }}.</p>
                </div>
